...
- Phase 2 giữ discovery trong cửa hàng: bộ lọc catalog, URL query shareable, empty/not-found states; không có trang `/category` hoặc `/artist`.
...
20. As a customer, I want to filter the store by one or more artists, so that I can narrow results to relevant creators.
21. As a customer, I want artist filters to use OR semantics, so that products from any selected artist are included.
22. As a customer, I want to filter by original-price range, so that I can shop within my budget using the API's documented price basis.
23. As a customer, I want an in-stock-only filter, so that I can hide products with no available variant.
24. As a customer, I want to sort products by `oldest`, so that I can discover the earliest catalog entries.
25. As a customer, I want existing newest and price sorting preserved, so that discovery does not regress.
...
- Test frontend rendered behavior with Vitest/jsdom: customer login/category/artist routes absent, guest checkout requests, confirmation, tracking form, query payload, loading/error/empty states, minimal tracking rendering, catalog query URLs, product gallery, related products, and voucher breakdown.
...
- Test Phase 2 API behavior for OR artist filters, original-price ranges, stock filter, and `oldest`.
...
- Category/artist storefront pages, new category-tree API, category CRUD, artist CRUD, product publishing, arbitrary sort values, undocumented query fields, or a broad storefront redesign.
...
- The backend category/artist APIs remain available for admin or future use, but this customer storefront does not expose category/artist routes.
